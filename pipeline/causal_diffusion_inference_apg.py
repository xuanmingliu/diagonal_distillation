...
from utils.apg import APGuidance


class CausalDiffusionInferencePipelineAPG(torch.nn.Module):
    def __init__(
            self,
            args,
            device,
            ...
            vae=None,
            apg_eta=0.1,
            apg_rescale_radius=None,
            apg_momentum_beta=-0.1
    ):
        super().__init__()
        ...
        self.apg = APGuidance(
            eta=apg_eta,
            rescale_radius=apg_rescale_radius,
            momentum_beta=apg_momentum_beta,
            device=device
        )

        print(f"KV inference with {self.num_frame_per_block} frames per block")
        print(f"APG enabled with eta={apg_eta}, rescale_radius={apg_rescale_radius}, momentum_beta={apg_momentum_beta}")
    ...
    def inference(
        ...
        """
        Perform inference on the given noise and text prompts using APG instead of CFG.
        """
        # Reset APG momentum for new generation sequence
        self.apg.reset_momentum()
        ...
        for current_num_frames in all_num_frames:
            ...
            for _, t in enumerate(tqdm(sample_scheduler.timesteps)):
                ...
                # Apply APG instead of standard CFG
                flow_pred = self.apg(
                    conditional_pred=flow_pred_cond,
                    unconditional_pred=flow_pred_uncond,
